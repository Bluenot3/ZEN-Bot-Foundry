...
const SidebarItem = ({ to, icon: Icon, label }: { to: string; icon: any; label: string }) => (
  <NavLink
    to={to}
    className={({ isActive }) =>
      `flex items-center gap-3 px-4 py-2.5 transition-all duration-300 group relative mx-2 rounded-xl mb-1 ${
        isActive
  ...
  >
    {({ isActive }) => (
      <>
        <Icon className={`w-3.5 h-3.5 transition-transform duration-300 ${isActive ? 'text-blue-400' : 'group-hover:scale-110'}`} />
      ...
      <aside className="w-56 bg-slate-900/50 backdrop-blur-3xl border-r border-white/5 z-40 flex flex-col shrink-0">
        <div className="p-5 border-b border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.4)]">
              <Logo className="text-white w-4 h-4" />
            </div>
            <div className="overflow-hidden">
              <h1 className="text-[12px] font-black text-slate-100 tracking-tighter leading-none uppercase">Zen Foundry</h1>
              <div className="flex items-center gap-1.5 mt-1.5">
                <div className="w-1 h-1 rounded-full bg-emerald-500 status-pulse"></div>
                <span className="text-[8px] text-emerald-500 font-bold uppercase tracking-widest">Core Secure</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 space-y-4 custom-scrollbar">
          <section>
            <div className="px-6 pb-1 text-[8px] font-black text-slate-500 tracking-[0.3em] uppercase">Intelligence</div>
          ...
          <section>
            <div className="px-6 pb-1 text-[8px] font-black text-slate-500 tracking-[0.3em] uppercase">Resources</div>
        ...
        <div className="p-5 border-t border-white/5 bg-black/20 shrink-0">
          ...
          <button 
            ...
            className="flex items-center gap-3 font-mono text-[9px] font-bold text-slate-500 hover:text-rose-400 transition-colors w-full px-2"
          >
