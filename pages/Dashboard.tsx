  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-10">
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6">
        <div className="space-y-1">
           <div className="flex items-center gap-2">
              <div className="w-1 h-1 rounded-full bg-blue-500 status-pulse shadow-[0_0_8px_rgba(59,130,246,0.8)]"></div>
              <div className="text-blue-500 font-mono text-[8px] font-bold tracking-[0.4em] uppercase">Tactical Neural Hub</div>
           </div>
           <h1 className="text-3xl md:text-4xl font-black text-slate-100 tracking-tighter uppercase leading-none">Command Center</h1>
        </div>
        <Link 
          to="/create"
          className="bg-white text-black px-5 py-3 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-blue-600 hover:text-white transition-all shadow-xl flex items-center gap-2 active:scale-95 group"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        ...
        <MetricBox label="TKN_THROUGHPUT" value={`${(totalTokens / 1000).toFixed(1)}K`} sub="Network Load" icon={Zap} color="text-cyan-400" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {bots.length === 0 ? (
          <div className="sm:col-span-full h-48 mil-panel flex flex-col items-center justify-center bg-white/[0.02] border-dashed border-white/10 rounded-2xl">
            <Terminal className="text-slate-800 mb-3" size={32} />
            <p className="text-[9px] font-black text-slate-600 tracking-[0.4em] uppercase">No Neutral Assets Identified</p>
            <Link to="/create" className="mt-4 text-blue-500 font-bold hover:text-blue-400 transition-colors uppercase tracking-widest text-[8px] border-b border-blue-500/30 pb-0.5">Begin First Synthesis &rarr;</Link>
          </div>
        ) : (
          bots.map(bot => (
